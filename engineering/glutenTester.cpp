...
uint32_t len = 4000000000;

int main(){
    T_char *str = new T_char[len];
    //for(int k = 0; k < 10; k++){
      for(uint32_t i = 0; i < len; i++)
          str[i] = rand() % 256;
      T_uint *suf = new T_uint[len];
      gettimeofday(&st, NULL);
      neo_sais_uchar(str, suf, len);
      gettimeofday(&ed, NULL);
      double time = ed.tv_sec - st.tv_sec + double(ed.tv_usec - st.tv_usec) / 1000000;
      std::cout << "all: " << time << std::endl;
      time = rest.tv_sec - st.tv_sec + double(rest.tv_usec - st.tv_usec) / 1000000;
      std::cout << "pre-rec: " << time << std::endl;
      time = ed.tv_sec - reed.tv_sec + double(ed.tv_usec - reed.tv_usec) / 1000000;
      std::cout << "post-rec: " << time << std::endl;
    //}
    return 0;
}